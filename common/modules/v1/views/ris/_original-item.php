...
<tr>
    ...
    <td>
    <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::button('<i class="fa fa-edit"></i> Edit Item', ['value' => Url::to(['/v1/ris/update-item', 'id' => $model->id, 'activity_id' => $item['activityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Original']), 'class' => 'btn btn-primary btn-xs btn-block', 'id' => 'update-'.$item['id'].'-button']) : '' ?>
    <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::a('<i class="fa fa-trash"></i> Delete Item', ['delete-item', 'id' => $model->id, 'activity_id' => $item['activityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Original'], [
        'class' => 'btn btn-xs btn-danger btn-block',
        'data' => [
