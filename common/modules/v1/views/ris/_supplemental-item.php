...
<tr>
    ...
    <?php if(!isset($specifications[$item['id']])){ ?>
      <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::button('<i class="fa fa-plus"></i> Create Spec', ['value' => Url::to(['/v1/ris/create-specification', 'id' => $model->id, 'activity_id' => $item['activityId'], 'sub_activity_id' => $item['subActivityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental']), 'class' => 'btn btn-info btn-block btn-xs', 'id' => 'create-specification-'.$item['id'].'-button']) : '' ?>
    <?php }else{ ?>
      <?= $specifications[$item['id']]->risItemSpecValueString ?><br><br>
      <div class="row">
        <div class="col-md-6 col-xs-12">
          <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::button('<i class="fa fa-edit"></i> Edit Spec', ['value' => Url::to(['/v1/ris/update-specification', 'id' => $model->id, 'activity_id' => $item['activityId'], 'sub_activity_id' => $item['subActivityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental']), 'class' => 'btn btn-primary btn-xs btn-block', 'id' => 'update-specification-'.$item['id'].'-button']) : '' ?>
        </div>
        <div class="col-md-6 col-xs-12">
          <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::a('<i class="fa fa-trash"></i> Delete Spec', ['delete-specification', 'id' => $model->id, 'activity_id' => $item['activityId'], 'sub_activity_id' => $item['subActivityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental'], [
    ...
    <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::a('<i class="fa fa-trash"></i> Delete Item', ['delete-item', 'id' => $model->id, 'activity_id' => $item['activityId'], 'sub_activity_id' => $item['subActivityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental'], [
        'class' => 'btn btn-xs btn-danger btn-block',
        'data' => [
