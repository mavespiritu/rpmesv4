...
<tr>
    ...
    <?php if(!isset($specifications[$item['id']])){ ?>
      <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::button('<i class="fa fa-upload"></i> Attach Specs', ['value' => Url::to(['/v1/ris/attach-specification', 'id' => $model->id, 'activity_id' => $item['activityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental']), 'class' => 'btn btn-info btn-block btn-xs', 'id' => 'attach-specification-'.$item['id'].'-button']) : '' ?>
    <?php }else{ ?>
      <?= \file\components\AttachmentsTable::widget(['model' => $specifications[$item['id']]]) ?>
      <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::a('<i class="fa fa-trash"></i> Delete Specs', ['delete-specification', 'id' => $model->id, 'activity_id' => $item['activityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Original'], [
    ...
    <td>
    <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::button('<i class="fa fa-edit"></i> Edit Item', ['value' => Url::to(['/v1/ris/update-item', 'id' => $model->id, 'activity_id' => $item['activityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental']), 'class' => 'btn btn-primary btn-xs btn-block', 'id' => 'update-'.$item['id'].'-button']) : '' ?>
    <?= ($model->status->status == 'Draft' || $model->status->status == 'For Revision') ? Html::a('<i class="fa fa-trash"></i> Delete Item', ['delete-item', 'id' => $model->id, 'activity_id' => $item['activityId'], 'item_id' => $item['stockNo'], 'cost' => $item['cost'], 'type' => 'Supplemental'], [
        'class' => 'btn btn-xs btn-danger btn-block',
        'data' => [
...
<?php
    $script = '
        $(document).ready(function(){
            ...
            $("#attach-specification-'.$item['id'].'-button").click(function(){
