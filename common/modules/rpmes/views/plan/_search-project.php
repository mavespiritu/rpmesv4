...
<div class="pull-left">
    ...
    <?= Yii::$app->user->can('AgencyUser') ?
            $model->currentStatus == 'For further validation' ?
                $dueDate ? 
                    strtotime(date("Y-m-d")) <= strtotime($dueDate->due_date) ?
                        Html::a('<i class="fa fa-paper-plane-o"></i> Re-submit Form 1', ['submit', 'id' => $model->id], [
                            'class' => 'btn btn-success',
                            'data' => [
                                'confirm' => 'Are you sure want to re-submit this Form 1?',
                                'method' => 'post',
                            ],
                        ]) :
                    '' :
    ...
    <?= Yii::$app->user->can('Administrator') ?
            $model->currentStatus == 'Submitted' ?
                Html::button('<i class="fa fa-paper-plane-o"></i> Send Form 1 for further validation', ['value' => Url::to(['revert', 'id' => $model->id]), 'class' => 'btn btn-danger', 'id' => 'revert-button']) :
            '' :
        '';
    ?>
</div>
...
<?php
  Modal::begin([
    ...
    'size' => "modal-lg",
    'header' => Yii::$app->user->can('Administrator') ? '<div id="acknowledge-modal-header"><h4>Acknowledge Form 1</h4></div>' : '<div id="acknowledge-modal-header"><h4>View Acknowledgment</h4></div>',
...
<?php
  Modal::begin([
    'id' => 'revert-modal',
    'size' => "modal-lg",
    'header' => '<div id="revert-modal-header"><h4>Send Form 1 for further validation</h4></div>',
    'options' => ['tabindex' => false],
  ]);
