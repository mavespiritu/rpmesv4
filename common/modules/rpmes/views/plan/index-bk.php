    ...
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    <?= $dueDate ? strtotime(date("Y-m-d")) <= strtotime($dueDate->due_date) ? '<p><i class="fa fa-exclamation-circle"></i> '.$HtmlHelper->time_elapsed_string($dueDate->due_date).' to go before the deadline of submission of monitoring plan. Due date is '.date("F j, Y", strtotime($dueDate->due_date)).'</p>' : '<p style="color: red;"><i class="fa fa-exclamation-circle"></i> Submission of monitoring plan has ended '.$HtmlHelper->time_elapsed_string($dueDate->due_date).' ago. Due date is '.date("F j, Y", strtotime($dueDate->due_date)).'</p>' : '' ?>
                ...
                <div class="pull-right">
                    <?= ButtonDropdown::widget([
                        'label' => '<i class="fa fa-download"></i> Export Form 1',
                        'encodeLabel' => false,
                    ...
                    <?= Html::button('<i class="fa fa-print"></i> Print Form 1', [
