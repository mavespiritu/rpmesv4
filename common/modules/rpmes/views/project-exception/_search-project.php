...
<div class="pull-left">
                ...
                $dueDate ? 
                    strtotime(date("Y-m-d")) <= strtotime($dueDate->due_date) ?
                        Html::a('<i class="fa fa-paper-plane-o"></i> Submit Form 3', ['submit', 'id' => $model->id], [
                            ...
                            'data' => [
                                'confirm' => 'The data I encoded had been duly approved by my Agency Head. I am providing my name and designation in the appropriate fields as an attestation of my submission\'s data integrity. Are you sure you want to submit this Form 3?',
                                'method' => 'post',
                            ],
                        ]) :
                    '' :
    ...
    <?= Yii::$app->user->can('AgencyUser') ?
            $model->currentStatus == 'For further validation' ?
                $dueDate ? 
                    strtotime(date("Y-m-d")) <= strtotime($dueDate->due_date) ?
                        Html::a('<i class="fa fa-paper-plane-o"></i> Re-submit Form 3', ['submit', 'id' => $model->id], [
                            'class' => 'btn btn-success',
                            'data' => [
                                'confirm' => 'Are you sure want to re-submit this Form 3?',
                                'method' => 'post',
                            ],
                        ]) :
                    '' :
    ...
    <?= Yii::$app->user->can('Administrator') ?
            $model->currentStatus == 'Submitted' ?
                Html::button('<i class="fa fa-paper-plane-o"></i> Send Form 3 for further validation', ['value' => Url::to(['revert', 'id' => $model->id]), 'class' => 'btn btn-danger', 'id' => 'revert-button']) :
            '' :
        '';
    ?>
...
<?php
  Modal::begin([
    ...
    'size' => "modal-lg",
    'header' => Yii::$app->user->can('Administrator') ? '<div id="acknowledge-modal-header"><h4>Acknowledge Form 3</h4></div>' : '<div id="acknowledge-modal-header"><h4>View Acknowledgment</h4></div>',
...
<?php
  Modal::begin([
    'id' => 'revert-modal',
    'size' => "modal-lg",
    'header' => '<div id="revert-modal-header"><h4>Send Form 3 for further validation</h4></div>',
    'options' => ['tabindex' => false],
  ]);
