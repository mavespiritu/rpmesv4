...
class SiteController extends \yii\web\Controller
{
    ...
    public function actionProjectBreakdown($year, $quarter, $agency_id, $category_id, $sector_id, $sub_sector_id, $province_id, $fund_source_id)
    {
        $year = $year == ''? Date("Y") : $year;
        $quarter = $quarter == ''? 'Q1' : $quarter;

        $projectRaw = Plan::find()->select(['project_id'])->where(['year' => $year])->asArray()->all();
        $projectRaw = ArrayHelper::map($projectRaw, 'project_id', 'project_id');

        $physicalTargets = ProjectTarget::find()->where(['target_type' => 'Physical', 'year' => $year])->createCommand()->getRawSql();
        $physicalAccomps = PhysicalAccomplishment::find()->where(['year' => $year])->createCommand()->getRawSql();
        $accomps = Accomplishment::find()->select(['project_id', 'IF(sum(COALESCE(action, 0)) > 0, 1, 0) as isCompleted'])->where(['year' => $year])->groupBy(['project_id'])->createCommand()->getRawSql();
        ...
        $projectStatus = $projectStatus->leftJoin(['regionTitles' => '('.$regionTitles.')'], 'regionTitles.project_id = project.id');
        $projectStatus = $projectStatus->leftJoin(['provinceTitles' => '('.$provinceTitles.')'], 'provinceTitles.project_id = project.id');
        $projectStatus = $projectStatus->andWhere(['project.year' => $year, 'project.draft' => 'No']);
        $projectStatus = $projectStatus->andWhere(['project.id' => $projectRaw]);

        if($agency_id != '')
        {
            $projectStatus = $projectStatus->andWhere(['agency.id' => $agency_id]);
        }

        if($sector_id != '')
        {
            $projectStatus = $projectStatus->andWhere(['sector.id' => $sector_id]);
        }
        ...
        if($fund_source_id != '')
        {
            $projectStatus = $projectStatus->andWhere(['fund_source.id' => $fund_source_id]);
        }
        
        $projectStatus = $projectStatus->groupBy(['sectorTitle']);
        $projectStatus = $projectStatus->asArray()->all();

        //echo '<pre>'; print_r($projectStatus); exit;
    ...
    public function actionProjectFinancial($year, $quarter, $agency_id, $category_id, $sector_id, $sub_sector_id, $province_id, $fund_source_id)
    {
        $year = $year == ''? Date("Y") : $year;
        $quarter = $quarter == ''? 'Q1' : $quarter;
        ...
        $financialAccomps = FinancialAccomplishment::find()->where(['year' => $year])->createCommand()->getRawSql();
        ...
        $financialTargetTotalPerQuarter = 'IF(project.data_type <> "Cumulative",
                IF("'.$quarter.'" = "Q1", COALESCE(financialTargets.q1, 0),
                    IF("'.$quarter.'" = "Q2", COALESCE(financialTargets.q1, 0) + COALESCE(financialTargets.q2, 0),
                        IF("'.$quarter.'" = "Q3", COALESCE(financialTargets.q1, 0) + COALESCE(financialTargets.q2, 0) + COALESCE(financialTargets.q3, 0),
                        COALESCE(financialTargets.q1, 0) + COALESCE(financialTargets.q2, 0) + COALESCE(financialTargets.q3, 0) + COALESCE(financialTargets.q4, 0)
                        )
                    )
                )
            ,   
                IF("'.$quarter.'" = "Q1", COALESCE(financialTargets.q1, 0),
                    IF("'.$quarter.'" = "Q2", IF(COALESCE(financialTargets.q2, 0) = 0, COALESCE(financialTargets.q1, 0), COALESCE(financialTargets.q2, 0)),
                        IF("'.$quarter.'" = "Q3", IF(COALESCE(financialTargets.q3, 0) = 0, IF(COALESCE(financialTargets.q2, 0) = 0, COALESCE(financialTargets.q1, 0), COALESCE(financialTargets.q2, 0)), COALESCE(financialTargets.q3)),
                            IF(COALESCE(financialTargets.q4, 0) = 0, IF(COALESCE(financialTargets.q3, 0) = 0, IF(COALESCE(financialTargets.q2, 0) = 0, COALESCE(financialTargets.q1, 0), COALESCE(financialTargets.q2, 0)), COALESCE(financialTargets.q3)), COALESCE(financialTargets.q4, 0))
                        )
                    )
                )
        ...
        $projectFinancial = Project::find()
            ...
            $projectFinancial = $projectFinancial->andWhere(['project.year' => $year, 'project.draft' => 'No']);
            $projectFinancial = $projectFinancial->andWhere(['project.id' => $projectRaw]);

        if($agency_id != '')
        {
            $projectFinancial = $projectFinancial->andWhere(['agency.id' => $agency_id]);
        }

        if($sector_id != '')
        {
            $projectFinancial = $projectFinancial->andWhere(['sector.id' => $sector_id]);
        }
        ...
        if($fund_source_id != '')
        {
            $projectFinancial = $projectFinancial->andWhere(['fund_source.id' => $fund_source_id]);
        }
        ...
        if($projectFinancial)
        {
            foreach($projectFinancial as $financial)
                {
                    $scriptFinancial .= "{ category: '".$financial['category']."', value: ".(($financial['value'] / $totalAllocation) * 1000)."}, ";
                }
        }

        return $this->renderAjax('_financial', [
            'scriptFinancial' => $scriptFinancial,
        ]);

    }

    public function actionProjectEmployment($year, $quarter, $agency_id, $category_id, $sector_id, $sub_sector_id, $province_id, $fund_source_id)
    {
        $year = $year == ''? Date("Y") : $year;
        $quarter = $quarter == ''? 'Q1' : $quarter;
        ...
        $personEmployedAccomps = PersonEmployedAccomplishment::find()->where(['year' => $year])->createCommand()->getRawSql();
        ...
        $maleEmployedAccomp = 'IF("'.$quarter.'" = "Q1", COALESCE(personEmployedAccompsQ1.male, 0),
                IF("'.$quarter.'" = "Q2", COALESCE(personEmployedAccompsQ1.male, 0) + COALESCE(personEmployedAccompsQ2.male, 0),
                    IF("'.$quarter.'" = "Q3", COALESCE(personEmployedAccompsQ1.male, 0) + COALESCE(personEmployedAccompsQ2.male, 0) + COALESCE(personEmployedAccompsQ3.male, 0),
                    COALESCE(personEmployedAccompsQ1.male, 0) + COALESCE(personEmployedAccompsQ2.male, 0) + COALESCE(personEmployedAccompsQ3.male, 0) + COALESCE(personEmployedAccompsQ4.male, 0)
                    )
                )
            )';

        $femaleEmployedAccomp = 'IF("'.$quarter.'" = "Q1", COALESCE(personEmployedAccompsQ1.female, 0),
                    IF("'.$quarter.'" = "Q2", COALESCE(personEmployedAccompsQ1.female, 0) + COALESCE(personEmployedAccompsQ2.female, 0),
                        IF("'.$quarter.'" = "Q3", COALESCE(personEmployedAccompsQ1.female, 0) + COALESCE(personEmployedAccompsQ2.female, 0) + COALESCE(personEmployedAccompsQ3.female, 0),
                        COALESCE(personEmployedAccompsQ1.female, 0) + COALESCE(personEmployedAccompsQ2.female, 0) + COALESCE(personEmployedAccompsQ3.female, 0) + COALESCE(personEmployedAccompsQ4.female, 0)
                        )
                    )
                )';
        ...
        $projectEmployment = $projectEmployment->leftJoin(['categoryTitles' => '('.$categoryTitles.')'], 'categoryTitles.project_id = project.id');
        $projectEmployment = $projectEmployment->leftJoin(['provinceTitles' => '('.$provinceTitles.')'], 'provinceTitles.project_id = project.id');
        $projectEmployment = $projectEmployment->andWhere(['project.year' => $year, 'project.draft' => 'No']);
        $projectEmployment = $projectEmployment->andWhere(['project.id' => $projectRaw]);

        if($agency_id != '')
        {
            $projectEmployment = $projectEmployment->andWhere(['agency.id' => $agency_id]);
        }

        if($sector_id != '')
        {
            $projectEmployment = $projectEmployment->andWhere(['sector.id' => $sector_id]);
        }
        ...
        if($fund_source_id != '')
        {
            $projectEmployment = $projectEmployment->andWhere(['fund_source.id' => $fund_source_id]);
        }
